...
TIME_DIFF_PATH = "time_differences.csv"
...
def log_times(sentinel_img, landsat_img, diff_seconds):
    
    with time_diff_lock:
        sentinel_time = sentinel_img.get("system:time_start").getInfo()
        landsat_time = landsat_img.get("system:time_start").getInfo()

        sentinel_time = datetime.utcfromtimestamp(sentinel_time / 1000)
        landsat_time = datetime.utcfromtimestamp(landsat_time / 1000)
